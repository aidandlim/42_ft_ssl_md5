...
void	sha384_set(t_sha384 *t, int i)
{
	...
	while (j < 16)
	{
		...
		while (k < 8)
		{
			g_chunk[j] += (u_int64_t)t->set[i * 128 + j * 8 + k] << (8 * (7 - k));
			k++;
		}
		j++;
	}
	while (j < 80)
	{
		s1 = RR_64(g_chunk[j - 15], 1) ^ RR_64(g_chunk[j - 15], 8) ^ (g_chunk[j - 15] >> 7);
		s2 = RR_64(g_chunk[j - 2], 19) ^ RR_64(g_chunk[j - 2], 61) ^ (g_chunk[j - 2] >> 6);
